...
function stripLegacyDescriptionComponent(
...
export const command: CommandMessage = {
  ...
  cooldown: 20,
  description:
    "Crea un nuevo bloque/embedded con editor interactivo (DisplayComponents).",
  category: "Alianzas",
  ...
  run: async (message, args, client) => {
    ...
    if (!blockName) {
      await message.reply(
        "Debes proporcionar un nombre. Uso: `!crear-embed <nombre>`"
      );
      return;
    }
    ...
    //@ts-ignore
    const channelSend: If<boolean, GuildTextBasedChannel, TextBasedChannel> =
      message.channel;
    ...
    const editorMessage = await channelSend.send({
      content:
        "⚠️ **IMPORTANTE:** Prepara tus títulos, descripciones y URLs antes de empezar.\n" +
    ...
    await handleEditorInteractions(
      editorMessage,
      message,
      client,
      blockName,
    ...
    case "edit_thumbnail": {
      ...
      try {
        await interaction.reply({
          flags: MessageFlags.Ephemeral,
          content:
            "Selecciona el bloque de texto al que quieres editar el thumbnail:",
          components: [
            {
              type: 1,
              components: [
      ...
      selCollector.on("collect", async (sel: any) => {
          ...
          fields: [
            {
              ...
              placeholder:
                "https://ejemplo.com/thumbnail.png (vacío para eliminar)",
        ...
        try {
          ...
          logger.error(
            { err: error },
            "No se pudo mostrar el modal de thumbnail"
          );
          return;
        }
        ...
        const rawInput = modalInteraction.components
    ...
    case "move_block": {
      ...
      selCollector.on("collect", async (sel: any) => {
        ...
        const btnCollector = replyMsg.createMessageComponentCollector({
          ...
          max: 1,
          time: 60000,
          filter: (b: any) => b.user.id === originalMessage.author.id,
        });
...
async function awaitModalWithDeferredReply(
  ...
  options: Parameters<ButtonInteraction["awaitModalSubmit"]>[0] = {
    time: 300000,
  }
...
async function handleAddSeparator(
  ...
  if (!wasAcknowledged) {
    try {
      ...
      logger.warn(
        { err: error },
        "No se pudo diferir respuesta al añadir separador"
      );
    }
  }
...
async function handleAddImage(
  ...
  try {
    ...
    const imageUrl = modalInteraction.components
...
async function handleCoverImage(
  ...
  try {
    ...
    const coverUrl = modalInteraction.components
...
async function handleShowVariables(
  interaction: ButtonInteraction
): Promise<void> {
